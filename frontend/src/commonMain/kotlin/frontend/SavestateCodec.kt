...
class SavestateCodec(
    ...
    fun decode(bytes: ByteArray): NesHardwareState = mapper.fromDto(cbor.decodeFromByteArray(SavestateDto.serializer(), bytes))
}
...
class SavestateStateMapper {
    ...
    private fun NesMachineState.toDto() = NesMachineStateDto(ppuMasterClockRemainder, previousNmiLine, cyclesUntilInputPoll)
    private fun NesMachineStateDto.toState() = NesMachineState(ppuMasterClockRemainder, previousNmiLine, cyclesUntilInputPoll)

    private fun CpuState.toDto() = CpuStateDto(pc, a, x, y, sp, status, totalCycles, nmiPending, irqLine, irqPending, irqSample, halted)
    private fun CpuStateDto.toState() = CpuState(pc, a, x, y, sp, status, totalCycles, nmiPending, irqLine, irqPending, irqSample, halted)
    ...
    private fun PpuStateDto.toState() = PpuState(
        frameColorIds = frameColorIds.toTypedArray(),
        renderFramebufferIndex = renderFramebufferIndex,
        completedFramebufferIndex = completedFramebufferIndex,
        oam = oam,
        ctrl = ctrl,
    ...
    private fun DmcStateDto.toState() = DmcState(values, flags)
    ...
    private fun MapperState.toDto(): MapperStateDto = when (this) {
        ...
        is Mapper4State -> MapperStateDto.Mapper4(chr, prgRam, registers, selectedRegister, prgMode, chrMode, irqLatch, irqCounter, irqReload, irqEnabled, irqRequested, mirroring, prgRamEnabled, prgRamWriteProtected)
    ...
    private fun MapperStateDto.toState(): MapperState = when (this) {
        ...
        is MapperStateDto.Mapper4 -> Mapper4State(chr, prgRam, registers, selectedRegister, prgMode, chrMode, irqLatch, irqCounter, irqReload, irqEnabled, irqRequested, mirroring, prgRamEnabled, prgRamWriteProtected)
...
@Serializable data class NesMachineStateDto(val ppuMasterClockRemainder: Int, val previousNmiLine: Boolean, val cyclesUntilInputPoll: Int)
@Serializable data class CpuStateDto(val pc: Int, val a: Int, val x: Int, val y: Int, val sp: Int, val status: Int, val totalCycles: Long, val nmiPending: Boolean, val irqLine: Boolean, val irqPending: Boolean, val irqSample: Boolean, val halted: Boolean)
...
@Serializable data class PpuStateDto(
    ...
    val nmiLine: Boolean, val secondaryOam: ByteArray, val activeSpriteX: IntArray, val activeSpriteAttributes: IntArray,
