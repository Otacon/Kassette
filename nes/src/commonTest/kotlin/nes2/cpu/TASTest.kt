...
class TASTest : FreeSpec({
    ...
    "sets stack pointer to A AND X and stores that AND high byte plus one" {
        state.pc = 0x8000
        state.a = 0x3F
        state.x = 0x1F
        state.y = 0x01
        ...
        state.a shouldBe 0x3F
        state.x shouldBe 0x1F
        state.sp shouldBe 0x1F
